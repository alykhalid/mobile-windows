...
namespace trovebox.Delay
{
    ...
    public static class LowProfileImageLoader
    {
        ...
        private static void saveImagetoIsolatedStorage(BitmapImage img, String UriText)
        {
            string UriSource = GenerateSlug(UriText);
            // Create virtual store and file stream. Check for duplicate tempJPEG files.
            using (IsolatedStorageFile myIsolatedStorage = IsolatedStorageFile.GetUserStoreForApplication())
            {
                if (myIsolatedStorage.FileExists(UriSource))
                {
                    myIsolatedStorage.DeleteFile(UriSource);
                }

                IsolatedStorageFileStream fileStream = myIsolatedStorage.CreateFile(UriSource);
                    ...
                    Deployment.Current.Dispatcher.BeginInvoke(() =>
                    {
                        while (0 < pendingCompletions.Count)
                        {
                            ...
                            if (GetUriSource(pendingCompletion.Image) == pendingCompletion.Uri)
                            {
                                ...
                                saveImagetoIsolatedStorage(bitmap, pendingCompletion.Uri.ToString());
        ...
        private static void OnUriSourceChanged(DependencyObject o, DependencyPropertyChangedEventArgs e)
        {
            ...
            else
            {
                ...
                using (IsolatedStorageFile myIsolatedStorage = IsolatedStorageFile.GetUserStoreForApplication())
                {
                    string UriSource = GenerateSlug(uri.ToString());
                    if (myIsolatedStorage.FileExists(UriSource))
                    {

                        using (IsolatedStorageFileStream fileStream = myIsolatedStorage.OpenFile(UriSource, FileMode.Open, FileAccess.Read))
                        {
